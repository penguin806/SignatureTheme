...
<!-- Header -->
    ...
    <header class="masthead signature-phil" style="background: <?php echo esc_attr($signature_options['phil-nav-bg-color']);?>">
      <div class="container-fluid">
          <div class="row">
              <article class="col-md-6 col-md-offset-3 text-center">
                  <a href="<?php echo esc_url(home_url( '/' )); ?>"><img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" title="<?php echo esc_attr(get_bloginfo('name')); ?>" class="main-logo img-responsive" src="<?php echo esc_url($signature_options['phil-nav-logo']['url']); ?>"/></a>
              </article>
          </div>
          <div class="row">
              <article class="col-md-6 col-md-offset-3 text-center">
                  <nav id="nav-main" class="mastnav signature-phil navigation dark font3" style="border-top: <?php echo esc_attr($signature_options['phil-nav-border-color']);?> solid 2px;">
                      <?php
                        if($signature_options['navigation_opt'] == "0")
                        {
                          $nav_args = array(
                            ...
                            'container'       => false,
                            'menu_class'      => 'main-menu signature-phil default-wp-nav',
                            'echo'            => true,
...
function signature_phil_nav(){
  ...
  else
  {
    ...
    else
    {
      ...
      $return = '<ul class="main-menu signature-phil '.esc_attr($nav_class).'">' . "\n";
      ...
      foreach ( $menunu as $i => $men ){
        if($men->menu_item_parent == '0')
        {
            ...
            $return .= '<a href="'. esc_url($href) .'" '.esc_attr($link_target).' class="main-link dark font3 '.esc_attr($identifyClass.$parent_link_class).'">'. esc_html($men->title) .'</a>';
