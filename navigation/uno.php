...
?>
    ...
    <header class="masthead signature-uno" style="background: <?php echo esc_attr($signature_options['uno-nav-bg-color']);?>">
      <div class="container">
          <div class="row">
              <article class="col-md-8 col-md-offset-2 text-center">
                  <a href="<?php echo esc_url(home_url( '/' )); ?>"><img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" title="<?php echo esc_attr(get_bloginfo('name')); ?>" class="main-logo img-responsive" src="<?php echo esc_url($signature_options['uno-nav-logo']['url']); ?>"/></a>
                  <nav id="nav-main" class="mastnav signature-uno navigation white font3" style="border-color: <?php echo esc_attr($signature_options['uno-nav-border-color']);?>">
                      <?php
                        if($signature_options['navigation_opt'] == "0")
                        {
                          $nav_args = array(
                            ...
                            'container'       => false,
                            'menu_class'      => 'main-menu signature-uno default-wp-nav',
                            'echo'            => true,
...
function signature_uno_nav(){
  ...
  else
  {
    ...
    else
    {
      ...
      $return = '<ul class="main-menu signature-uno '.esc_attr($nav_class).'">' . "\n";
      ...
      foreach ( $menunu as $i => $men ){
        if($men->menu_item_parent == '0')
        {
            ...
            $return .= '<a href="'. esc_url($href) .'" '.esc_attr($link_target).' class="main-link font3 white '.esc_attr($identifyClass.$parent_link_class).'">'. esc_html($men->title) .'</a>';
