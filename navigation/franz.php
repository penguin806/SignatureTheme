...
<!-- Header -->
    ...
    <header class="masthead signature-franz" style="background: <?php echo esc_attr($signature_options['franz-nav-bar-bg-color']);?>">
      <div class="container">
        <div class="row">
          <article class="col-md-6 text-left">
            <a href="<?php echo esc_url(home_url( '/' )); ?>"><img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" title="<?php echo esc_attr(get_bloginfo('name')); ?>" class="main-logo img-responsive" src="<?php echo esc_url($signature_options['franz-nav-bar-logo']['url']); ?>"/></a>
          </article>
          <article class="col-md-6 text-right">
            <a class="menu-trigger" href="#"><span class="ion-navicon dark"></span></a>
          </article>
        </div>
      </div>
    </header>
    ...
    <nav class="mastnav signature-franz fullheight">

      <div class="menu-close-notification">
          <a class="font2" href="#"><span class="ion-android-close black"></span></a>
      </div>
      <div class="mastnav-inner-left fullheight"></div>

      <div class="mastnav-inner-right fullheight" style="background: <?php echo esc_attr($signature_options['franz-nav-panel-bg-color']);?>">
        <div class="valign">
            
          <a href="<?php echo esc_url(home_url( '/' )); ?>"><img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" title="<?php echo esc_attr(get_bloginfo('name')); ?>" class="main-logo img-responsive" src="<?php echo esc_url($signature_options['franz-nav-logo']['url']); ?>"/></a>
          <?php
              if($signature_options['navigation_opt'] == "0")
              {
                $nav_args = array(
                  ...
                  'container'       => false,
                  'menu_class'      => 'main-menu signature-franz default-wp-nav',
                  'echo'            => true,
...
function signature_franz_nav(){
  ...
  else
  {
    ...
    else
    {
      ...
      $return = '<ul class="main-menu signature-franz '.esc_attr($nav_class).'">' . "\n";
      ...
      foreach ( $menunu as $i => $men ){
        if($men->menu_item_parent == '0')
        {
            ...
            $return .= '<a href="'. esc_url($href) .'" '.esc_attr($link_target).' class="main-link font2 '.esc_attr($identifyClass.$parent_link_class).'">'. esc_html($men->title) .'</a>';
            ...
            if($has_sub_menu == 1)
            {
              ...
              foreach ( $menunu as $submenu )
              {
                if($submenu->menu_item_parent == $men->ID)
                {
                  ...
                  $return .= '<li><a href="'. esc_url($href) .'" '.esc_attr($link_target).' class="font3 '.esc_attr($identifyClass).'">'. esc_html($submenu->title) .'</a></li>';
