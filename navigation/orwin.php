...
?>
    ...
    <header class="masthead signature-igor">
      <div class="container-fluid"> 
        <div class="row">
          
            <article class="col-md-6">
              <a href="<?php echo esc_url(home_url( '/' )); ?>"><img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" title="<?php echo esc_attr(get_bloginfo('name')); ?>" class="nav-logo img-responsive" src="<?php echo esc_url($signature_options['orwin-nav-bar-logo']['url']); ?>"/></a>
            </article>
            <article class="col-md-6 notification-icon-wrap">
                  <div class="menu-notification">
                      <a class="font2" href="#"><span class="ion-navicon black"></span></a>
                  </div>
    ...
    <nav class="mastnav signature-igor fullheight text-center">
        <div class="menu-close-notification">
          <a class="font2" href="#"><span class="ion-android-close white"></span></a>
        </div>
        
        <div class="valign orwin-nav">
            <?php
              if($signature_options['navigation_opt'] == "0")
              {
                $nav_args = array(
                  ...
                  'container'       => false,
                  'menu_class'      => 'main-menu signature-igor default-wp-nav',
                  'echo'            => true,
...
function signature_orwin_nav(){
  ...
  else
  {
    ...
    else
    {
      ...
      $return = '<ul class="main-menu signature-igor '.esc_attr($nav_class).'">' . "\n";
      ...
      foreach ( $menunu as $i => $men ){
        if($men->menu_item_parent == '0')
        {
            ...
            $return .= '<a href="'. esc_url($href) .'" '.esc_attr($link_target).' class="main-link font1 white '.esc_attr($identifyClass.$parent_link_class).'">'. esc_html($men->title) .'</a>';
            ...
            if($has_sub_menu == 1)
            {
              ...
              foreach ( $menunu as $submenu )
              {
                if($submenu->menu_item_parent == $men->ID)
                {
                  ...
                  $return .= '<li><a href="'. esc_url($href) .'" '.esc_attr($link_target).' class="font1 white '.esc_attr($identifyClass).'">'. esc_html($submenu->title) .'</a></li>';
