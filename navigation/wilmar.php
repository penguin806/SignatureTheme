    ...
    <header class="masthead signature-wilmar white-bg">
      <div class="masthead-inner">
        <a href="<?php echo esc_url(home_url( '/' )); ?>">
          <img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" title="<?php echo esc_attr(get_bloginfo('name')); ?>" class="main-logo img-responsive" src="<?php echo esc_url($signature_options['wilmar-nav-logo']['url']); ?>"/>
        </a>
        <p class="slogan font3 grey">
          <?php 
            echo wp_kses($signature_options['wilmar-nav-content'], array(
                        'a' => array(
                            'href' => array(),
                            'title' => array(),
                            'target' => array()
          ...
          <?php
            if($signature_options['navigation_opt'] == "0")
            {
              $nav_args = array(
                ...
                'container'       => false,
                'menu_class'      => 'main-menu signature-wilmar default-wp-nav',
                'echo'            => true,
...
function signature_wilmar_nav(){
  ...
  {
    $return = '<h5 class="clearfix font3 dark '.esc_attr($nav_class).'">'.esc_html__('Please configure the navigation menu','signature').'</h5>';
  }
  else
  {
    ...
    else
    {
      ...
      $return = '<ul class="main-menu signature-wilmar '.esc_attr($nav_class).'">' . "\n";
      ...
      foreach ( $menunu as $i => $men ){
        if($men->menu_item_parent == '0')
        {
            ...
            $return .= '<a href="'. esc_url($href) .'" '.esc_attr($link_target).' class="main-link font3 dark '.esc_attr($identifyClass.$parent_link_class).'">'. esc_html($men->title) .'</a>';
            ...
            if($has_sub_menu == 1)
            {
              ...
              foreach ( $menunu as $submenu )
              {
                if($submenu->menu_item_parent == $men->ID)
                {
                  ...
                  $return .= '<li><a href="'. esc_url($href) .'" '.esc_attr($link_target).' class="font3 grey '.esc_attr($identifyClass).'">'. esc_html($submenu->title) .'</a></li>';
