...
<!-- Header -->
    ...
    <header class="masthead signature-gozzo" style="background: <?php echo esc_attr($signature_options['gozzo-nav-bg-color']);?>">
      <div class="container">
          <div class="row">
              <article class="col-md-4 text-left">
                  <a href="<?php echo esc_url(home_url( '/' )); ?>"><img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" title="<?php echo esc_attr(get_bloginfo('name')); ?>" class="main-logo img-responsive" src="<?php echo esc_url($signature_options['gozzo-nav-logo']['url']); ?>"/></a>
              </article>
                      ...
                      <?php
                        if($signature_options['navigation_opt'] == "0")
                        {
                          $nav_args = array(
                            ...
                            'container'       => false,
                            'menu_class'      => 'main-menu signature-gozzo default-wp-nav',
                            'echo'            => true,
...
function signature_gozzo_nav(){
  ...
  else
  {
    ...
    else
    {
      ...
      $return = '<ul class="main-menu signature-gozzo '.esc_attr($nav_class).'">' . "\n";
      ...
      foreach ( $menunu as $i => $men ){
        if($men->menu_item_parent == '0')
        {
            ...
            $return .= '<a href="'. esc_url($href) .'" '.esc_attr($link_target).' class="main-link font3 '.esc_attr($identifyClass.$parent_link_class).'">'. esc_html($men->title) .'</a>';
