...
?>
    ...
    <header class="masthead signature-amor fullheight" style="background: <?php echo esc_attr($signature_options['amor-nav-bg-color']);?>">
      <div class="valign">
        
      <div class="masthead-inner">
        <a href="<?php echo esc_url(home_url( '/' )); ?>">
          <?php
          if($signature_options['amor-nav-logo']['url'] != '')
          {
          ?>
            <img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" title="<?php echo esc_attr(get_bloginfo('name')); ?>" class="nav-logo img-responsive" src="<?php echo esc_url($signature_options['amor-nav-logo']['url']); ?>"/>
          <?php
          }
          else
        ...
        <nav class="mastnav signature-amor">
          <?php
            ...
            if( $signature_options['navigation_opt'] == "0")
            {
              $nav_args = array(
                ...
                'container'       => false,
                'menu_class'      => 'main-menu signature-amor default-wp-nav',
                'echo'            => true,
...
function signature_amor_nav(){
  ...
  else
  {
    ...
    else
    {
      ...
      $return = '<ul class="main-menu signature-amor '.esc_attr($nav_class).'">' . "\n";
      ...
      foreach ( $menunu as $i => $men ){
        if($men->menu_item_parent == '0')
        {
            ...
            $return .= '<a href="'. esc_url($href) .'" '.esc_attr($link_target).' class="main-link font3bold dark '.esc_attr($identifyClass.$parent_link_class).'">'. esc_html($men->title) .'</a>';
