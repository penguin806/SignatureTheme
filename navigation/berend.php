...
?>
<!-- Header -->
    <header class="masthead signature-berend" style="background: <?php echo esc_attr($signature_options['berend-nav-bg-color']);?>">
      <div class="container">
        <div class="row">
          <article class="col-md-6 text-left">
            <a href="<?php echo esc_url(home_url( '/' )); ?>"><img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" title="<?php echo esc_attr(get_bloginfo('name')); ?>" class="main-logo img-responsive" src="<?php echo esc_url($signature_options['berend-nav-bar-logo']['url']); ?>"/></a>
          </article>
    ...
    <nav class="mastnav signature-berend fullheight">
        ...
        <div class="mastnav-inner-left fullheight img-bg text-center">
          <div class="valign">
            <a href="<?php echo esc_url(home_url( '/' )); ?>"><img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" title="<?php echo esc_attr(get_bloginfo('name')); ?>" class="nav-logo img-responsive" src="<?php echo esc_url($signature_options['berend-nav-logo']['url']); ?>"/></a>
          </div>
        </div>

        <div class="mastnav-inner-right fullheight" style="background: <?php echo esc_attr($signature_options['berend-nav-bg-color']); ?>;">
          <div class="valign">
            <?php
              if($signature_options['navigation_opt'] == "0")
              {
                $nav_args = array(
                  ...
                  'container'       => false,
                  'menu_class'      => 'main-menu signature-berend default-wp-nav',
                  'echo'            => true,
            ...
            <div class="menu-credits text-left">
              <p class="font1 dark">
                <?php 
                  echo wp_kses($signature_options['berend-nav-content'], array(
                                    'a' => array(
                                        'href' => array(),
                                        'title' => array(),
                                        'target' => array()
...
function signature_berend_nav(){
  ...
  else
  {
    ...
    else
    {
      ...
      $return = '<ul class="main-menu signature-berend '.esc_attr($nav_class).'">' . "\n";
      ...
      foreach ( $menunu as $i => $men ){
        if($men->menu_item_parent == '0')
        {
            ...
            $return .= '<a href="'. esc_url($href) .'" '.esc_attr($link_target).' class="main-link dark font2 '.esc_attr($identifyClass.$parent_link_class).'">'. esc_html($men->title) .'</a>';
            ...
            if($has_sub_menu == 1)
            {
              ...
              foreach ( $menunu as $submenu )
              {
                if($submenu->menu_item_parent == $men->ID)
                {
                  ...
                  $return .= '<li><a href="'. esc_url($href) .'" '.esc_attr($link_target).' class="black font4 '.esc_attr($identifyClass).'">'. esc_html($submenu->title) .'</a></li>';
