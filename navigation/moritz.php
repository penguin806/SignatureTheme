...
?>
    ...
    <header class="masthead signature-moritz moritz-trans-header" style="background: <?php echo esc_attr($signature_options['moritz-nav-bar-bg-color']);?>">
      <div class="container-fluid"> 
        <div class="row">
          
            <article class="col-md-6">
              <a href="<?php echo esc_url(home_url( '/' )); ?>"><img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" title="<?php echo esc_attr(get_bloginfo('name')); ?>" class="nav-logo img-responsive" src="<?php echo esc_url($signature_options['moritz-nav-bar-logo']['url']); ?>"/></a>
            </article>
    ...
    <nav class="mastnav signature-moritz fullheight" style=" background-color:<?php echo esc_attr($signature_options['moritz-nav-bg-color']);?>; background-image: url(<?php echo esc_url($signature_options['moritz-nav-bg-img']['url']);?>);">
          ...
          <?php
            if($signature_options['navigation_opt'] == "0")
            {
              $nav_args = array(
                ...
                'container'       => false,
                'menu_class'      => 'main-menu signature-moritz default-wp-nav',
                'echo'            => true,
            ...
            {
              signature_moritz_nav();
            }
          ?>

          <div class="menu-credits text-left">
              <p class="font3" style="color: <?php echo esc_attr($signature_options['moritz-nav-content-color']); ?>;">
                <?php 
                  echo wp_kses($signature_options['moritz-nav-content'], array(
                                    'a' => array(
                                        'href' => array(),
                                        'title' => array(),
                                        'target' => array()
...
function signature_moritz_nav(){
  ...
  else
  {
    ...
    else
    {
      ...
      $return = '<ul class="main-menu signature-moritz '.esc_attr($nav_class).'">' . "\n";
