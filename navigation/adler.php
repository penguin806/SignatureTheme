...
?>
    ...
    <!-- Header -->
    <header class="masthead signature-adler" style="background: <?php echo esc_attr($signature_options['adler-nav-bg-color']);?>">
      <div class="masthead-inner">
        <a href="<?php echo esc_url(home_url( '/' )); ?>">
          <?php
          if($signature_options['adler-nav-logo']['url'] != '')
          {
          ?>
            <img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" title="<?php echo esc_attr(get_bloginfo('name')); ?>" class="nav-logo img-responsive" src="<?php echo esc_url($signature_options['adler-nav-logo']['url']); ?>"/>
          <?php
          }
          else
          ...
          <?php
            ...
            if( $signature_options['navigation_opt'] == "0")
            {
              $nav_args = array(
                ...
                'container'       => false,
                'menu_class'      => 'main-menu signature-adler default-wp-nav',
                'echo'            => true,
...
function signature_adler_nav(){
  ...
  else
  {
    ...
    else
    {
      ...
      $return = '<ul class="main-menu signature-adler '.esc_attr($nav_class).'">' . "\n";
            ...
            if($has_sub_menu == 1)
            {
              ...
              foreach ( $menunu as $submenu )
              {
                if($submenu->menu_item_parent == $men->ID)
                {
                  ...
                  $return .= '<li><a href="'. esc_url($href) .'" '.esc_attr($link_target).' class="'.esc_attr($identifyClass).'">'. esc_html($submenu->title) .'</a></li>';
